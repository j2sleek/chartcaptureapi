...
export function registerAuth(app) {
    if (!authEnabled) {
        log.warn("API_KEYS is empty — authentication is DISABLED. Set API_KEYS in production.");
        return;
    }
    ...
    app.addHook("onRequest", async (request) => {
        ...
        if (!key || !keys.has(key)) {
            throw new UnauthorizedError("Missing or invalid API key (send X-API-Key or Authorization: Bearer <key>)");
        }
    });
}
