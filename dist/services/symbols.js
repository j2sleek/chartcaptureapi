...
function normalizeMetric(metric) {
    ...
    if (!KNOWN_METRICS.has(cleaned)) {
        throw new UnknownResourceError(`Unknown metric "${metric}". Known metrics: ${[...KNOWN_METRICS].join(", ")}`);
    }
    return cleaned;
}
export function resolveTarget(symbol, metric) {
    ...
    if (raw.includes("_")) {
        ...
        if (!pageSlug) {
            throw new UnknownResourceError(`Unsupported symbol "${symbol}"; add its page slug to symbols.ts`);
        }
        return {
    ...
    if (!ticker) {
        throw new UnknownResourceError(`Unknown symbol "${symbol}". Known: ${Object.keys(SLUG_TO_TICKER).join(", ")}`);
    }
    const pageSlug = TICKER_TO_PAGE_SLUG[ticker];
    return {
        metricPath: `/${pageSlug}/${metricSlug}/`,
